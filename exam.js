...
function submitExam() {
  ...
  const st = loadState();
  addXpToState(st, xpEarned);
  st.ok         = (st.ok || 0) + correct;
  st.wrongCount = (st.wrongCount || 0) + wrong;
  ...
  /* XP popup */
  if (typeof showXpFloat === 'function') showXpFloat(xpEarned);

  /* Check badges */
  if (typeof checkAndAwardBadges === 'function') checkAndAwardBadges(loadState());
}
...
/* ══ تهيئة شاشة الامتحان عند الدخول ══ */
function initExam() {
  _showExamView('setup');
}
...
document.addEventListener('DOMContentLoaded', function() {
  var es = document.getElementById('examScreen');
  if (!es) return;
  var obs = new MutationObserver(function() {
    if (es.classList.contains('active')) initExam();
  });
  obs.observe(es, { attributes: true, attributeFilter: ['class'] });
});
