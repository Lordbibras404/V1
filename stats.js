/* ============================================================
   GRIT Learn v9.1 — Stats & Analytics Module (Fixed)
   ============================================================ */
'use strict';
...
const StatsModule = (() => {
  ...
  function buildMastery() {
    ...
    const subjects = [
      ...
      {key:'tamazight', icon:'ⵣ',  name:'أمازيغية',   color:'#f472b6'},
    ];
    return `
      <div class="glass-card" style="margin:0 16px;padding:16px;border-radius:18px">
        ...
        ${subjects.map(s => {
          const pct = Math.min(100, Math.round(mastery[s.key] || 0));
          return `
  ...
  function buildOverview() {
    ...
    const lvlInfo  = (typeof getLevelInfo === 'function') ? getLevelInfo(xp) : { name:'برونزي', pct:0, needed:1000 };
    ...
    return `
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;padding:0 16px">
        ...
        ${ring(lvlInfo.idx !== undefined ? lvlInfo.idx + 1 : 1, 6, '#f59e0b', 'المستوى')}
      ...
      <div style="margin:0 16px;padding:14px 16px;background:rgba(139,92,246,.08);border:1px solid rgba(139,92,246,.18);border-radius:16px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          <span style="font-size:.8rem;font-weight:800;color:var(--text1)">⚡ ${lvlInfo.name}</span>
          <span style="font-size:.72rem;color:var(--text3)">${xp.toLocaleString()} XP</span>
        </div>
        <div style="height:6px;background:rgba(255,255,255,.07);border-radius:3px;overflow:hidden">
          <div style="height:100%;width:${lvlInfo.pct}%;background:linear-gradient(90deg,var(--accent),#818cf8);border-radius:3px;transition:width 1.2s ease"></div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:0 16px">
        ${[
          ...
          {icon:'❌', val:wrong.toLocaleString(),       lbl:'إجابة خاطئة', color:'#ef4444'},
